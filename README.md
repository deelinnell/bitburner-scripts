...
These are my scripts I've written so far. The main module finds all the servers on the network and cracks each one that it is able to crack. The grow, weaken, and hack scripts are then loaded on the servers. A target is then chosen to be hacked by all your servers on the network. There are many variables to consider, but max money and hacking level required are the most important.
