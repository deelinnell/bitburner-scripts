...
export async function main(ns) {
  ...
  // Returns array with each server with viruses loaded.
      return vulnerable;
  }
  ...
  function hackTarget(target, currentMoney) {
      const money = currentMoney ? currentMoney : ns.getServerMoneyAvailable(target);
  ...
  // Main Attack Loop
  async function mainAttackCycle(target, security, currentMoney) {
  ...
  // and the server changes from each attack.
      ...
      const hacked = hackTarget(target);
  ...
  // Calculate the delay required per attack to prevent overlap.
      ...
      const weakDelay = currentDelay * 2;
      const growDelay = weaken2.time + weakDelay - growth.time + currentDelay;
      const hackDelay = weaken1.time - hacked.time + currentDelay;
  ...
  // Start attack
      attackLoop.map(attack => attackPerScript(attack, target));
  // Use recursion to begin calculations for next attack.
      // mainAttackCycle(target, growth.security, currentMoney);
  }
  ...
  // If not enough threads - WAIT.
      ns.print(`====== ${cycleThreadsRequired} ======`);
      ns.print(`====== ${availableThreads} ======`);
  ...
  // Get available servers / threads for each attack.
      ...
      network.map(node => {
          loopThroughAvailableNetwork(target, script, node, threads);
      });
  }

  function loopThroughAvailableNetwork(target, script, node, requiredThreads) {
